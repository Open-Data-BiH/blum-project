...
export function setupTimetableSelection(): void {
  ...
  Promise.all(timetablePromises)
    ...
    .catch((error) => {
      ...
      `;
      timetableDisplay.querySelector<HTMLButtonElement>('.retry-btn')?.addEventListener('click', setupTimetableSelection);
    });
}
...
export function loadTimetable(lineId: string): void {
  ...
  const lang = getCurrentLanguage();
  const loadingText =
    safeGet(getTranslations(), lang, 'sections', 'timetable', 'loading') || 'Loading timetable...';
  ...
  fetch(timetableFile)
    ...
    .catch((error) => {
      ...
      `;
      timetableDisplay.querySelector<HTMLButtonElement>('.retry-btn')?.addEventListener('click', () => loadTimetable(lineId));
    });
}
...
function renderTimetable(timetable: TimetableEntry & { lineType?: string }, container: HTMLElement): void {
  ...
  const sundayHolidayLabelText =
    (timetableDays?.sundayHoliday) ||
    (lang === 'bhs' ? 'Nedjelja i praznik' : 'Sunday and Holiday');
  ...
  container.querySelectorAll<HTMLElement>('.direction-btn').forEach((button) => {
    button.addEventListener('click', function (this: HTMLElement) {
      ...
      const activeDay = container.querySelector<HTMLElement>('.day-btn.active')?.getAttribute('data-day') ?? 'weekday';
  ...
  container.querySelectorAll<HTMLElement>('.day-btn').forEach((button) => {
    button.addEventListener('click', function (this: HTMLElement) {
      ...
      const activeDirection = container.querySelector<HTMLElement>('.direction-btn.active')?.getAttribute('data-direction') ?? directionAId;
...
const scrollToCurrentHour = (): void => {
  ...
  if (!visibleView) {
    visibleView = Array.from(document.querySelectorAll<HTMLElement>('.timetable-view')).find(
...
const updateTimeHighlighting = (): void => {
  ...
  let visibleViews = Array.from(document.querySelectorAll<HTMLElement>('.timetable-view:not([style*="display: none"])'));
