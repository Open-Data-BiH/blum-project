...
export class GeolocationService {
  ...
  findNearestStops(markers: MarkerLike[], limit = 3): Array<{ marker: MarkerLike; distance: number; lat: number; lng: number }> {
    ...
    const calculated = markers.map((marker) => {
      ...
      const distance = this.calculateDistance(this.userLocation!.lat, this.userLocation!.lng, latlng.lat, latlng.lng);
