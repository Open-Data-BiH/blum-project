...
const loadTransportHubs = (L: LeafletNS, map: Map, hubs: TransportHub[], groups: Record<OverlayLayerId, LayerGroup>): void => {
  hubs.forEach((hub) => {
...
const loadBikeStations = (L: LeafletNS, map: Map, stations: BikeStation[], group: LayerGroup): void => {
  stations.forEach((station) => {
    ...
    marker.on('click', () => window.setTimeout(() => createWalkingCircles(L, map, [station.lat, station.lon]), 100));
    marker.addTo(group);
  });
};
